	...
	private void HandleDataAttributes(HtmlNode node)
	{
		if (node.HasAttributes && node.Attributes.Count(x => x.Name.ToLower().Contains("data-")) > 0)
		{
			...
			foreach (var dataAttribute in dataAttributes)
			{
				...
				{
					if (dataAttribute.Value.StartsWith("url('"))
					{
						var regex = new Regex(@"url\('\s*(?<url>.*?)\s*'\)");
						if (regex.IsMatch(dataAttribute.Value))
						{
							var match = regex.Match(dataAttribute.Value);
							if (match.Groups.Count == 3)
							{
								var url = match.Groups[2].Value;
								node.Attributes[dataAttribute.Name].Value = dataAttribute.Value.Replace(url, GetUrl(url));
							}
						}
					}
					else
